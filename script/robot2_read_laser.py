...
#		self.image_received = False
		...
		rospy.logwarn("[Robot2] LaserRead Node [ONLINE]...")

		# rospy shutdown
		rospy.on_shutdown(self.cbShutdown)
			
		# Subscribe to LaserScan msg
		self.laser_topic = "/scan_robot2"
...
#					rospy.loginfo("[Angle: 0]: %.4f" % (self.scanValue[0]))
					...
					rospy.logwarn("[Robot2] OBSTACLE! [Angle: 0]: %.4f" % (self.scanValue[center]))
		else:
	...
	def cbShutdown(self):
		...
		rospy.logerr("[Robot2] LaserRead Node [OFFLINE]...")

if __name__ == '__main__':

	# Initialize
	rospy.init_node('robot2_read_scan', anonymous=False)
	laser = LaserPreview()
